use crate::subr::{
    calc_auto_correlation, calc_cluster, calc_correlation, calc_difference_average,
    calc_difference_entropy, calc_difference_variance, calc_id, calc_idm, calc_idmn, calc_idn,
    calc_imc1, calc_imc2, calc_inverse_var, calc_joint_energy, calc_joint_entropy,
    calc_marginal_col_prob, calc_marginal_row_prob, calc_marginals_inplace, calc_max_prob,
    calc_mcc, calc_mean_gray_intensity, calc_sum_average, calc_sum_entropy, calc_sum_of_squares,
    in_box, in_volume, symmetrize_in_place,
};
...
thread_local! {
    static THREAD_GLCM: RefCell<Vec<f64>> = const { RefCell::new(vec![]) };
}
...
pub fn map_glcm_features(
    ...
    thread_pool.install(|| {
        ...
        features
            .par_chunks_exact_mut(n_features)
            .enumerate()
            .for_each(|(g_idx, feature_set)| {
                    ...
                    for ii in -pr..=pr {
                        for jj in -pr..=pr {
                            for kk in -pr..=pr {
                                ...
                                let neighbor_virtual_idx = [
                                    root_idx[0] as i32 + r[0],
                                    root_idx[1] as i32 + r[1],
                                    root_idx[2] as i32 + r[2],
                                ];
                ...
                THREAD_GLCM.with(|cell| {
                    ...
                    glcm.chunks_exact_mut(n_bins * n_bins)
                        .for_each(|glcm| symmetrize_in_place(glcm, n_bins));
                    ...
                    px.chunks_exact_mut(n_bins)
                        .zip(glcm.chunks_exact(n_bins * n_bins))
                        .for_each(|(px, glcm)| {
                            calc_marginal_row_prob(glcm, n_bins, px);
                        });
                    py.chunks_exact_mut(n_bins)
                        .zip(glcm.chunks_exact(n_bins * n_bins))
                        .for_each(|(py, glcm)| {
                    ...
                    // let sig_px:Vec<f64> = px.chunks_exact(n_bins).map(std_dev).collect();
                    // let sig_py:Vec<f64> = py.chunks_exact(n_bins).map(std_dev).collect();
                    ...
                    if to_calculate.contains(&AUTO_CORRELATION) {
                        feature_set[AUTO_CORRELATION as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .map(|glcm| calc_auto_correlation(glcm, n_bins))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&JOINT_AVERAGE) {
                        feature_set[JOINT_AVERAGE as usize] =
                            ux.iter().sum::<f64>() / n_angles as f64;
                    }

                    if to_calculate.contains(&CLUSTER_PROMINENCE)
                        || to_calculate.contains(&CLUSTER_SHADE)
                        || to_calculate.contains(&CLUSTER_TENDENCY)
                        || to_calculate.contains(&CLUSTER_PROMINENCE)
                    {
                        // features 3 - 6
                        let mut prom = 0.;
                        let mut shade = 0.;
                        ...
                        glcm.chunks_exact(n_bins * n_bins)
                            .zip(ux.iter().zip(&uy))
                            .for_each(|(glcm, (&ux, &uy))| {
                                let [prom_, shade_, tend_, cont_] =
                                    calc_cluster(glcm, n_bins, ux, uy);
                                prom += prom_;
                                shade += shade_;
                                tend += tend_;
                                cont += cont_;
                    ...
                    if to_calculate.contains(&CORRELATION) {
                        feature_set[CORRELATION as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .zip(ux.iter().zip(&uy))
                            .map(|(glcm, (&ux, &uy))| calc_correlation(glcm, n_bins, ux, uy, eps))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&DIFFERENCE_AVERAGE) {
                        feature_set[DIFFERENCE_AVERAGE as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_difference_average(pxmy, &kvald))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&DIFFERENCE_ENTROPY) {
                        feature_set[DIFFERENCE_ENTROPY as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_difference_entropy(pxmy, eps))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&DIFFERENCE_VARIANCE) {
                        feature_set[DIFFERENCE_VARIANCE as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_difference_variance(pxmy, &kvald))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&JOINT_ENERGY) {
                        feature_set[JOINT_ENERGY as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .map(|glcm| calc_joint_energy(glcm))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&JOINT_ENTROPY) {
                        feature_set[JOINT_ENTROPY as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .map(|glcm| calc_joint_entropy(glcm, eps))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&IMC1) {
                        feature_set[IMC1 as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .zip(&hxy)
                    ...
                    if to_calculate.contains(&IMC2) {
                        feature_set[IMC2 as usize] = hxy
                            .iter()
                            .zip(px.chunks_exact(n_bins).zip(py.chunks_exact(n_bins)))
                            .map(|(&hxy, (px, py))| calc_imc2(px, py, hxy, eps))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&INVERSE_DIFFERENCE_MOMENT) {
                        feature_set[INVERSE_DIFFERENCE_MOMENT as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_idm(pxmy, &kvald))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&INVERSE_DIFFERENCE_MOMENT_NORMALIZED) {
                        feature_set[INVERSE_DIFFERENCE_MOMENT_NORMALIZED as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_idmn(pxmy, &kvald, n_bins))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&INVERSE_DIFFERENCE) {
                        feature_set[INVERSE_DIFFERENCE as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_id(pxmy, &kvald))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&INVERSE_DIFFERENCE_NORMALIZED) {
                        feature_set[INVERSE_DIFFERENCE_NORMALIZED as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_idn(pxmy, &kvald, n_bins))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&INVERSE_VARIANCE) {
                        feature_set[INVERSE_VARIANCE as usize] = pxmy
                            .chunks_exact(n_bins)
                            .map(|pxmy| calc_inverse_var(pxmy, &kvald))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&MAXIMUM_PROBABILITY) {
                        feature_set[MAXIMUM_PROBABILITY as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .map(|glcm| calc_max_prob(glcm))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&SUM_AVERAGE) {
                        feature_set[SUM_AVERAGE as usize] = pxpy
                            .chunks_exact(n_pxpy)
                            .map(|pxpy| calc_sum_average(pxpy, &kvals))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&SUM_ENTROPY) {
                        feature_set[SUM_ENTROPY as usize] = pxpy
                            .chunks_exact(n_pxpy)
                            .map(|pxpy| calc_sum_entropy(pxpy, eps))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
                    ...
                    if to_calculate.contains(&SUM_OF_SQUARES) {
                        feature_set[SUM_OF_SQUARES as usize] = glcm
                            .chunks_exact(n_bins * n_bins)
                            .zip(&ux)
                            .map(|(glcm, &ux)| calc_sum_of_squares(glcm, n_bins, ux))
                            .sum::<f64>()
                            / n_angles as f64;
                    }
