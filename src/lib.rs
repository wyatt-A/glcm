...
use crate::subr::{auto_correlation, calc_id, calc_idm, calc_idmn, calc_idn, calc_imc1, calc_imc2, calc_inverse_var, calc_joint_entropy, calc_marginal_col_prob, calc_marginal_row_prob, calc_marginals_inplace, calc_max_prob, calc_mcc, calc_mean_gray_intensity, calc_sum_average, calc_sum_entropy, calc_sum_of_squares, cluster, correlation, difference_average, difference_entropy, difference_variance, get_range_inclusive, in_bounds, joint_energy, joint_entropy, std_dev, symmetrize_in_place_f64};
use rayon::prelude::*;
...
mod tests {
    ...
    fn test_eig() {
        ...
        let x = ArrayView2::from_shape((3,3).f(),&m).unwrap();

        let e = x.eigvals().unwrap();
    ...
    fn test_pyrad_comparison() {
        ...
        map_glcm(&dims, &x, n_features, &mut features, &angles, n_bins, patch_radius, &[[1,1,1]]);

        for i in 0..n_features {
            let f_addr = f_dims.calc_addr(&[i,1,1,1]); // middle voxel
    ...
    features.par_chunks_exact_mut(n_features).enumerate().for_each(|(g_idx,feature_set)|{
        ...
        THREAD_GLCM.with(|cell|{
            ...
            glcm.chunks_exact(n_bins*n_bins).zip(ux.iter().zip(&uy)).zip(sig_px.iter().zip(&sig_py)).for_each(|((glcm,(&ux,&uy)),(&s_px,&s_py))|{
                ...
                corr += correlation(glcm,n_bins,ux,uy,eps);
            });
            ...
            let mut imc1 = 0.;
            let mut imc2 = 0.;
            glcm.chunks_exact(n_bins*n_bins).zip(&hxy)
            .zip(px.chunks_exact(n_bins).zip(py.chunks_exact(n_bins)))
            .for_each(|((glcm,&hxy),(px,py))|{
                //let [imc1_,imc2_] = calc_imc(glcm,n_bins,px,py,eps);
                imc1 += calc_imc1(glcm,n_bins,px,py,hxy,eps);
                imc2 += calc_imc2(px,py,hxy,eps);
                //imc1 += imc1_;
                //imc2 += imc2_;
            });
            ...
            // let dur = now.elapsed();
            // println!("idm took {} us",dur.as_micros());
